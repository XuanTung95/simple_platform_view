...
public class SimplePlatformViewsChannel {
   ...
   */
  public SimplePlatformViewsChannel(@NonNull BinaryMessenger dartExecutor) {
    channel =
        new MethodChannel(dartExecutor, "tungpx/simple_platform_views", StandardMethodCodec.INSTANCE);
    channel.setMethodCallHandler(parsingHandler);
  }

  /**
   * Sets the {@link SimplePlatformViewsHandler} which receives all events and requests that are parsed
   * from the underlying platform views channel.
   */
  public void setPlatformViewsHandler(@Nullable SimplePlatformViewsHandler handler) {
    this.handler = handler;
  }

  /**
   * Handler that receives platform view messages sent from Flutter to Android through a given
   * {@link SimplePlatformViewsChannel}.
   *
   * <p>To register a {@code PlatformViewsHandler} with a {@link SimplePlatformViewsChannel}, see {@link
   * SimplePlatformViewsChannel#setPlatformViewsHandler(SimplePlatformViewsHandler)}.
   */
  public interface SimplePlatformViewsHandler {
